	...
	*/
	...
	initSliders: function(){
		...
		self._box.find('li.mfilter-filter-item.mfilter-slider').each(function(i){
			var _this		= jQuery(this).attr('data-slider-id', i),
				...
				$min		= _this.find('.ui-slider-handle1 input'),
				$max		= _this.find('.ui-slider-handle2 input'),
	...
	_parseInfo: function( data, first ) {
		var self	= this,
			...
			if(Object.keys(filters).length){
				$('.mfilter-button-reset').css('display','block');
			}
	...
	__initLoader: function() {
		...
		this._jqLoader = jQuery('<span class="mfilter-ajax-loader-container" style="cursor: wait; z-index: 100; margin: 0; padding: 0; position: absolute; text-align: center; background-color: rgba(' + color + ',0.7);"></span>')
			...
			.html( '<div class="load__preloader__row">' +
		...
		self._box.find('.mfilter-filter-item').each(function(i){
			...
			if( ! isContentTop && heading.hasClass( 'mfilter-collapsed' ) ) {
				if( typeof params[seo_name] == 'undefined' ) {
					jQuery(heading).parents('.mfilter-filter-item').addClass('noactive');
					content.show();
				}
			}
					...
					function init( first ) {
						...
						if( count ) {
							...
							function sh( show, force ) {
								...
								_level0.find('a.mfilter-button-more').toggleClass( 'mfilter-button-more-open' ).text(show?self._options.displayListOfItems.textMore.replace( '%s', count ):self._options.displayListOfItems.textLess);
							}
	...
	_initHeading: function() {
		...
		self._box.find('.mfilter-heading').click(function(){
			...
			if( jQuery(this).hasClass('mfilter-collapsed') ) {
				...
				jQuery(this).parents('.mfilter-filter-item').removeClass('noactive');
			} else {
				...
				jQuery(this).parents('.mfilter-filter-item').addClass('noactive');
			}
		});
	},
		...
		for( i in filters ) {
			(function($li){
				...
				switch( true ) {
					case type == 'slider' : {
						(function( $inputs ){
							var $min = $inputs.find('.ui-slider-handle1 input'),
								$max = $inputs.find('.ui-slider-handle2 input');
	...
	_initPrice: function() {
		...
		slider.slider({
			...
			slide	: function( e, ui ) {
				...
				$('#custom-handle1 span').text( ui.values[0] );
				$('#custom-handle2 span').text( ui.values[1] );
			},
			create: function() {
				$('#custom-handle1 span').text( minInput.val() );
				$('#custom-handle2 span').text( maxInput.val() );
			},
			stop	: function( e, ui ) {
				if( self._options['refreshResults'] != 'using_button' )
					self.runAjax();
